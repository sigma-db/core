type Schema = { [name: string]: number };		// relation name --> arity
type Tuple = number[];
type Relation = Tuple[];
type Index = Relation;
type Database = { [name: string]: Relation };	// relation name --> relation
type Atom = { name: string, vars: string[] };
type Query = { head: Atom, body: Atom[] };
type QueryFunction = (db: Database) => Relation;
type Interval = number;
type Box = Interval[];

const L = 10;
const MIN: number = 0;          // inclusive
const MAX: number = 1 << L;     // exclusive
const WILDCARD: number = 1;
 ...
 */
function* dyadic(start: number, end: number): IterableIterator<number> {
 ...
 */
function gaps_db(Q: Query, D: Database, SAO: string[]): Box[] {
    let B: Box[] = [];
    Q.body.forEach(atom => {
 ...
 * @param b Second box
 */
function resolve(a: Box, b: Box): Box {
 ...
 * @param Q The query to infer the schema from
 */
function schema(Q: Query): Schema {
    return Q.body.reduce((S: Schema, atom: Atom) => {
...
export function query(Q: string): QueryFunction {
    const _Q: Query = parse(Q);
    const SAO: string[] = _Q.head.vars;         // TODO
    const all: Box = Array(_Q.head.vars.length).fill(WILDCARD);     // a box covering the entire search space
    const is_tuple = (b: Box) => b.every(i => (i & MAX) == MAX);    // checks whether a given box is a tuple
    ...
    return (D: Database) => {
        ...
        let probe = (b: Box): [boolean, Box | Tuple] => {
